...
df <- randomSimulation(n, 15, df = df, seed = 3)
```
...
There are 3 general first moves, those (on the $3\times 3$ magic square) being, 8, 1, and 5. All of the other first moves can be written as a rotation or reflection of these three
Let's simulate 1,000 games using each of the afro 

```{r}
board8 <- c(rep(NA,7), 1, NA)
board1 <- c(1, rep(NA,8))
board5 <- c(rep(NA,4),1,rep(NA,4))

df1 <- data.frame(matrix(ncol = 2, nrow = 0))
colnames(df) <- c('Winner', 'Board Size')

df1 <- randomSimulation(n, 3, board8, df = df1, seed = 0, 'First Move')
df1 <- randomSimulation(n, 3, board1, df = df1, seed = 1, 'First Move')
